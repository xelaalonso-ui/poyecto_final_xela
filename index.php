...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    if (!empty($usuario) && !empty($contrasena)) {
        ...
        $usuario_limpio = mysqli_real_escape_string($db, $usuario);
        
        // Buscar usuario por username o email
        $sql = "SELECT * FROM Usuario WHERE username = '$usuario_limpio' OR email = '$usuario_limpio' LIMIT 1";
        ...
        if($resultado && mysqli_num_rows($resultado) > 0) {
            ...
            if(password_verify($contrasena, $user_data['password_hash'])) {
                $_SESSION['usuario'] = $user_data['username'];
                $_SESSION['id'] = $user_data['id_usuario'];
                $_SESSION['email'] = $user_data['email'];
                
                // Actualizar último login
                mysqli_query($db, "UPDATE Usuario SET ultimo_login = NOW() WHERE id_usuario = {$user_data['id_usuario']}");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['registro'])) {
    ...
    if (!empty($usuario) && !empty($correo) && !empty($contrasena)) {
        // Escapar datos
        $usuario_limpio = mysqli_real_escape_string($db, $usuario);
        $correo_limpio = mysqli_real_escape_string($db, $correo);
        
        // Hash de la contraseña
        $password_hash = password_hash($contrasena, PASSWORD_DEFAULT);
        
        // Verificar si el usuario o email ya existe
        $check_sql = "SELECT id_usuario FROM Usuario WHERE username = '$usuario_limpio' OR email = '$correo_limpio'";
        ...
        if(mysqli_num_rows($check_result) > 0) {
            ...
            $sql = "INSERT INTO Usuario (username, email, password_hash, estado_cuenta) 
                    VALUES ('$usuario_limpio', '$correo_limpio', '$password_hash', 'activo')";
            
            if(mysqli_query($db, $sql)) {
                $id_usuario_nuevo = mysqli_insert_id($db);
                ...
                if (isset($_FILES['foto_usuario']) && $_FILES['foto_usuario']['error'] === 0) {
                    ...
                    $nuevo_nombre = 'perfil_' . $id_usuario_nuevo . '_' . time() . '.' . $extension;
                    ...
                    if(move_uploaded_file($foto_tmp, $foto_destino)) {
                        ...
                        $sql_foto = "INSERT INTO Fotos (id_usuario, url_foto, descripcion, tipo_foto) 
                                     VALUES ($id_usuario_nuevo, '$foto_destino', 'Foto de perfil', 'perfil')";
